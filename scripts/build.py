...
Eseguito da GitHub Actions ad ogni push su ricette.txt
"""
...
CATEGORY_RULES = [
    ("Dolci & Dessert",       r"torta|dolce|cupcake|biscotto|castagnaccio|sablé|sable|zuppetta di fragole|panna cotta|financier|cremoso|mousse|crumble|crostata|frolla|sorbetto|pan di spagna|bavarese|chantilly|crema pasticciera|zabaione dolce|bicchiere composta"),
    ("Pizza & Focaccia",      r"pizza|focaccia|schiacciata"),
    ("Tacos & Street Food",   r"tacos|taco|nachos|cono|coni|tramezzini"),
    ("Panini & Piadine",      r"panino|piadina|bun |burger|schiacciata scarola"),
    ...
    ("Secondi Piatti",        r"filetto|polpo|polpette|cozze fritte|cavolo cappuccio viola|bocconcini di pollo|jarret|pollo marinato|fegatini|arancina|arancine|cecina gamberi|riso.nero|paccheri neri|cannolo al nero|spaghetto mayo|tonno cbt|maialino|ricciola olio|passata di ceci|polenta baccalà|polenta nero|cozze pesto|cozze fritte"),
    ("Zuppe & Vellutate",     r"zuppetta|zuppa|crema di|vellutata|chowder|brodetto|brodo|minestra riso|passata"),
    ("Antipasti & Stuzzichini", r".*"),  # catch-all
]
...
def unsplash_query(title: str) -> str:
    """Genera una query Unsplash sensata dal titolo italiano."""
    replacements = {
        ...
        "castagnaccio": "chestnut cake rustic",
    }
    q = title.lower()
    for it, en in replacements.items():
        if it in q:
            return en + " food photography"
    # Fallback: primi 3 parole del titolo + "italian food"
    words = title.split()[:3]
    return " ".join(words) + " italian food"
...
def parse_recipes(text: str) -> list[dict]:
    ...
    for line in text.splitlines():
        line = line.strip()
        # Salta righe vuote, commenti, intestazioni
        if not line or line.startswith("#"):
            continue
        # Righe con ingredienti/passi incorporati (es. castagnaccio con grammature)
        # Le trattiamo come ricette normali; il titolo è la prima parte
        title = line.split(".")[0].strip() if ". " not in line[:3] else line
        # Deduplica
        key = title.lower()[:40]
        if key in seen:
            continue
...
<style>
:root{{--bg:#0f0e0c;--surface:#1a1915;--surface2:#242320;--accent:#e8a838;--accent2:#d05a2f;--text:#f0ece4;--muted:#9a948a;--border:#2e2c28;}}
*,*::before,*::after{{box-sizing:border-box;margin:0;padding:0;}}
body{{background:var(--bg);color:var(--text);font-family:'DM Sans',sans-serif;min-height:100vh;overflow-x:hidden;}}
.hero{{position:relative;padding:80px 40px 60px;text-align:center;background:radial-gradient(ellipse 80% 60% at 50% 0%,rgba(232,168,56,.15) 0%,transparent 70%);border-bottom:1px solid var(--border);}}
...
.card-img-wrap .overlay{{position:absolute;inset:0;background:linear-gradient(to top,rgba(15,14,12,.6) 0%,transparent 55%);}}
...
.modal-note{{color:var(--muted);font-size:.9rem;line-height:1.7;font-style:italic;background:var(--surface2);border-left:3px solid var(--accent);padding:16px 20px;border-radius:0 8px 8px 0;margin-bottom:24px;}}
...
<header class="hero">
  ...
  <p>Una collezione personale in continuo aggiornamento — idee, esperimenti e grandi classici.</p>
  <div class="stats">
...
function buildCard(r) {{
  ...
  card.innerHTML = `
    ...
    <div class="card-footer">
      <span style="font-size:.75rem;color:var(--muted)">Clicca per la ricetta</span>
...
function openModal(r) {{
  ...
  fetchPhoto(r.id, r.query).then(url => {{
    if (url) {{ mp.src = url; }}
  }});
  document.getElementById('modalCat').textContent   = `${{CAT_EMOJI[r.cat]}} ${{r.cat}}`;
  document.getElementById('modalTitle').textContent = r.title;
  document.getElementById('modalNote').textContent  = r.note || 'Ricetta completa disponibile. Clicca le note per i dettagli.';
